'use client';

import { useCallback, useEffect, useState } from 'react';
import Link from 'next/link';
import { motion, AnimatePresence } from 'motion/react';
import { Plus, Trash2, Tag, Pencil } from 'lucide-react';
import { ProtectedRoute } from '../../../components/ProtectedRoute';
import { t } from '../../../lib/i18n';
import {
  listCategories,
  createCategory,
  updateCategory,
  deleteCategory,
  type CategoryDto,
} from '../../../lib/catalog';

const navItems = [
  { label: t.admin.navDashboard, href: '/admin' },
  { label: t.admin.navUsers, href: '/admin/users' },
  { label: t.admin.navProducts, href: '/admin/products' },
  { label: t.admin.navCategories, href: '/admin/categories', active: true },
  { label: t.adminDisputes.title, href: '/admin/disputes' },
  { label: t.admin.navCoupons, href: '/admin/coupons' },
];

const inputCls =
  'h-10 w-full rounded-lg border border-border bg-white px-3 text-sm outline-none focus:border-brand-500';

export default function AdminCategoriesPage() {
  const [categories, setCategories] = useState<CategoryDto[]>([]);
  const [loading, setLoading] = useState(true);
  const [toast, setToast] = useState<{ message: string; type: 'success' | 'error' } | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<CategoryDto | null>(null);
  const [toDelete, setToDelete] = useState<CategoryDto | null>(null);

  // Form state
  const [name, setName] = useState('');
  const [slug, setSlug] = useState('');
  const [description, setDescription] = useState('');
  const [icon, setIcon] = useState('');
  const [saving, setSaving] = useState(false);

  const showToast = (message: string, type: 'success' | 'error') => {
    setToast({ message, type });
    setTimeout(() => setToast(null), 3000);
  };

  const fetchCategories = useCallback(() => {
    setLoading(true);
    listCategories()
      .then(setCategories)
      .catch(() => showToast(t.adminCategories.loadError, 'error'))
      .finally(() => setLoading(false));
  }, []);

  useEffect(() => {
    fetchCategories();
  }, [fetchCategories]);

  const resetForm = () => {
    setName('');
    setSlug('');
    setDescription('');
    setIcon('');
    setEditing(null);
    setShowForm(false);
  };

  const openCreate = () => {
    resetForm();
    setShowForm(true);
  };

  const openEdit = (c: CategoryDto) => {
    setEditing(c);
    setName(c.name);
    setSlug(c.slug);
    setDescription(c.description ?? '');
    setIcon(c.icon ?? '');
    setShowForm(true);
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      showToast(t.adminCategories.formError, 'error');
      return;
    }
    setSaving(true);
    try {
      const payload = {
        name: name.trim(),
        slug: slug.trim() || undefined,
        description: description.trim() || undefined,
        icon: icon.trim() || undefined,
      };
      if (editing) {
        await updateCategory(editing._id, payload);
        showToast(t.adminCategories.updated, 'success');
      } else {
        await createCategory(payload);
        showToast(t.adminCategories.created, 'success');
      }
      resetForm();
      fetchCategories();
    } catch (err) {
      showToast(
        (err as { response?: { data?: { message?: string } } }).response?.data?.message ??
          t.adminCategories.actionError,
        'error',
      );
    }
    setSaving(false);
  };

  const toggleActive = async (c: CategoryDto) => {
    try {
      await updateCategory(c._id, { isActive: !c.isActive });
      showToast(t.adminCategories.updated, 'success');
      fetchCategories();
    } catch {
      showToast(t.adminCategories.actionError, 'error');
    }
  };

  const confirmDelete = async () => {
    if (!toDelete) return;
    try {
      await deleteCategory(toDelete._id);
      setToDelete(null);
      showToast(t.adminCategories.deleted, 'success');
      fetchCategories();
    } catch (err) {
      showToast(
        (err as { response?: { data?: { message?: string } } }).response?.data?.message ??
          t.adminCategories.actionError,
        'error',
      );
      setToDelete(null);
    }
  };

  return (
    <ProtectedRoute roles={['admin']}>
      <div className="flex min-h-screen" style={{ backgroundColor: 'var(--color-bg)' }}>
        {/* Sidebar */}
        <aside
          className="hidden w-[260px] shrink-0 flex-col border-r bg-white py-6 md:flex"
          style={{ borderColor: 'var(--color-border)' }}
        >
          <div className="flex items-center gap-2 px-5 pb-6">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-500 text-sm font-extrabold text-white">
              A
            </span>
            <span className="text-base font-bold" style={{ color: 'var(--color-text)' }}>
              {t.admin.panel}
            </span>
          </div>
          <nav className="flex flex-col gap-0.5 px-2">
            {navItems.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className={`flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${
                  item.active
                    ? 'bg-brand-50 font-semibold text-brand-600'
                    : 'text-[var(--color-text-muted)] hover:bg-[var(--color-bg)]'
                }`}
              >
                {item.label}
              </Link>
            ))}
          </nav>
        </aside>

        {/* Main */}
        <div className="flex flex-1 flex-col gap-5 overflow-auto p-8">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <h1 className="text-2xl font-bold" style={{ color: 'var(--color-text)' }}>
                {t.adminCategories.title}
              </h1>
              <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                {t.adminCategories.subtitle} · {t.adminCategories.count(String(categories.length))}
              </p>
            </div>
            {!showForm && (
              <button
                type="button"
                onClick={openCreate}
                className="flex items-center gap-1.5 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-bold text-white hover:bg-brand-600"
              >
                <Plus className="h-4 w-4" aria-hidden />
                {t.adminCategories.create}
              </button>
            )}
          </div>

          {/* Form */}
          <AnimatePresence>
            {showForm && (
              <motion.form
                initial={{ opacity: 0, height: 0 }}
                animate={{ opacity: 1, height: 'auto' }}
                exit={{ opacity: 0, height: 0 }}
                onSubmit={handleSubmit}
                className="overflow-hidden rounded-2xl border border-border bg-white p-5"
              >
                <h2 className="mb-4 text-base font-bold text-brand-900">
                  {editing ? t.adminCategories.editTitle : t.adminCategories.formTitle}
                </h2>
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="flex flex-col gap-1.5 text-sm font-medium text-text">
                    {t.adminCategories.fName}
                    <input
                      value={name}
                      onChange={(e) => setName(e.target.value)}
                      placeholder={t.adminCategories.fNamePlaceholder}
                      required
                      className={inputCls}
                    />
                  </label>
                  <label className="flex flex-col gap-1.5 text-sm font-medium text-text">
                    {t.adminCategories.fSlug}{' '}
                    <span className="text-muted">{t.adminCategories.optional}</span>
                    <input
                      value={slug}
                      onChange={(e) => setSlug(e.target.value)}
                      placeholder={t.adminCategories.fSlugPlaceholder}
                      className={inputCls}
                    />
                  </label>
                  <label className="flex flex-col gap-1.5 text-sm font-medium text-text">
                    {t.adminCategories.fIcon}{' '}
                    <span className="text-muted">{t.adminCategories.optional}</span>
                    <input
                      value={icon}
                      onChange={(e) => setIcon(e.target.value)}
                      className={inputCls}
                    />
                  </label>
                  <label className="flex flex-col gap-1.5 text-sm font-medium text-text sm:col-span-2">
                    {t.adminCategories.fDescription}{' '}
                    <span className="text-muted">{t.adminCategories.optional}</span>
                    <input
                      value={description}
                      onChange={(e) => setDescription(e.target.value)}
                      className={inputCls}
                    />
                  </label>
                </div>
                <div className="mt-4 flex gap-2">
                  <button
                    type="submit"
                    disabled={saving}
                    className="rounded-lg bg-brand-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-600 disabled:opacity-60"
                  >
                    {saving ? t.adminCategories.saving : t.adminCategories.save}
                  </button>
                  <button
                    type="button"
                    onClick={resetForm}
                    className="rounded-lg border border-border px-5 py-2.5 text-sm font-medium text-text"
                  >
                    {t.adminCategories.cancel}
                  </button>
                </div>
              </motion.form>
            )}
          </AnimatePresence>

          {/* Table */}
          {loading ? (
            <div className="rounded-xl border border-border bg-white p-12 text-center text-sm text-muted">
              {t.adminCategories.loading}
            </div>
          ) : categories.length === 0 ? (
            <div className="flex flex-col items-center gap-3 rounded-2xl border border-border bg-white py-16 text-center">
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-brand-50">
                <Tag className="h-7 w-7 text-brand-300" aria-hidden />
              </div>
              <p className="font-bold text-brand-900">{t.adminCategories.empty}</p>
              <p className="text-sm text-muted">{t.adminCategories.emptyDesc}</p>
            </div>
          ) : (
            <div className="overflow-hidden rounded-2xl border border-border bg-white">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left text-xs uppercase tracking-wide text-muted">
                    <th className="px-5 py-3 font-semibold">{t.adminCategories.colName}</th>
                    <th className="px-5 py-3 font-semibold">{t.adminCategories.colSlug}</th>
                    <th className="px-5 py-3 font-semibold">{t.adminCategories.colDescription}</th>
                    <th className="px-5 py-3 font-semibold">{t.adminCategories.colStatus}</th>
                    <th className="px-5 py-3 text-right font-semibold">
                      {t.adminCategories.colActions}
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {categories.map((c) => (
                    <tr key={c._id}>
                      <td className="px-5 py-3 font-bold text-brand-900">
                        {c.icon ? `${c.icon} ` : ''}
                        {c.name}
                      </td>
                      <td className="px-5 py-3 text-muted">{c.slug}</td>
                      <td className="max-w-xs truncate px-5 py-3 text-muted">
                        {c.description || '—'}
                      </td>
                      <td className="px-5 py-3">
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-bold ${
                            c.isActive ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'
                          }`}
                        >
                          {c.isActive ? t.adminCategories.active : t.adminCategories.inactive}
                        </span>
                      </td>
                      <td className="px-5 py-3">
                        <div className="flex items-center justify-end gap-3">
                          <button
                            type="button"
                            onClick={() => toggleActive(c)}
                            className="text-xs font-semibold text-brand-600 hover:underline"
                          >
                            {c.isActive ? t.adminCategories.disable : t.adminCategories.enable}
                          </button>
                          <button
                            type="button"
                            onClick={() => openEdit(c)}
                            aria-label={t.adminCategories.fName}
                            className="text-muted transition-colors hover:text-brand-600"
                          >
                            <Pencil className="h-4 w-4" />
                          </button>
                          <button
                            type="button"
                            onClick={() => setToDelete(c)}
                            aria-label={t.adminCategories.delete}
                            className="text-muted transition-colors hover:text-[var(--color-error)]"
                          >
                            <Trash2 className="h-4 w-4" />
                          </button>
                        </div>
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          )}
        </div>
      </div>

      {/* Delete confirm */}
      <AnimatePresence>
        {toDelete && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={() => setToDelete(null)}
            className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 p-4"
          >
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-sm rounded-2xl bg-white p-6"
            >
              <p className="font-semibold text-brand-900">
                {t.adminCategories.deleteConfirm(toDelete.name)}
              </p>
              <div className="mt-5 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setToDelete(null)}
                  className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-text"
                >
                  {t.adminCategories.cancel}
                </button>
                <button
                  type="button"
                  onClick={confirmDelete}
                  className="rounded-lg bg-[var(--color-error)] px-4 py-2 text-sm font-semibold text-white"
                >
                  {t.adminCategories.delete}
                </button>
              </div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Toast */}
      <AnimatePresence>
        {toast && (
          <motion.div
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className={`fixed bottom-6 right-6 z-50 rounded-lg px-5 py-3 text-sm font-medium text-white shadow-lg ${
              toast.type === 'success' ? 'bg-green-500' : 'bg-red-500'
            }`}
          >
            {toast.message}
          </motion.div>
        )}
      </AnimatePresence>
    </ProtectedRoute>
  );
}
